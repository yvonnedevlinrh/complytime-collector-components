...
- **beacon-distro** pre-built OTel Collector distribution (UBI10 Minimal container) bundling proofwatch, truthbeam, the AWS S3 exporter, OIDC and bearer token auth extensions, and OpenAPI request validation. Container images are cosign-signed with SBOM attestation and published to `ghcr.io` and `quay.io`.
